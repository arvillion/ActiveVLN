...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        model_inputs = {}
        should_add_generation_prompt = messages[-1]["role"] == "assistant"

        if self.processor is not None:
            from verl.utils.dataset.vision_utils import process_image, process_raw_image, process_video

            raw_prompt = self.processor.apply_chat_template(messages, add_generation_prompt=should_add_generation_prompt, tokenize=False)
            multi_modal_data = {}
            ...
            raw_prompt = self.tokenizer.apply_chat_template(messages, add_generation_prompt=should_add_generation_prompt, tokenize=False)
