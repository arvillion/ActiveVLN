...
import httpx
import time
from .utils.serial_utils import deserialize_observation, deserialize_step_result
...
class BatchEnvClient:
    ...
    def _make_request(self, endpoint: str, method: str = "POST", data: Any = None) -> Any:
        ...
        try:
            if method.upper() == "GET":
                ...
                response = httpx.delete(url, headers=headers, json=data, timeout=self.timeout)
            else:
    ...
    def warmup_batch(self, cfgs):
        self._make_request("batch/warmup", "POST", {"cfgs": cfgs})
